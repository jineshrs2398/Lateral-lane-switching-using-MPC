...
statesTotal=np.zeros((len(t),len(states))) # It will keep track of all the states during the entire manoeuvre
...
UTotal=np.zeros(len(t)) # To keep track all the inputs over time
UTotal[0]=U1
...
for i in range(0,sim_length-1):
    ...
    if k+outputs*hz<=len(refSignals):
        ...
        r=refSignals[k:len(refSignals)]  #r6 matrix which is then transposed
        hz=hz-1
...
plt.grid(True)
plt.legend(loc='upper right',fontsize='small')
plt.ylim(-X_ref[-1]/2,X_ref[-1]/2) # Scale roads
plt.show()


# Plot the input delta(t) and the outputs: psi(t) and Y(t)
plt.subplot(3,1,1)
