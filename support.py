...
class SupportFilesCar:
    ## The functions interact with the main file ##
    ...
    def __init__(self):
        ...
        Q=np.matrix('50 0;0 1') # weights for outputs (all samples, except the last one)
        S=np.matrix('50 0;0 1') # weights for the final horizon period outputs
        ...
        trajectory=2 #  choosing trajectory function between 1,2,3
    ...
    def trajectory_generator(self,t,r,f):
        ### This method creates the trajectory for the car to follow ###
        ...
        if trajectory==1:
            ...
            if aaa<0:
                ...
                bbb=-14
            y_1=aaa*(x+self.constants[17]-100)**2+bbb
        ...
        psi[1:len(psi)]=np.arctan2(dy[0:len(dy)],dx[0:len(dx)]) #psi=dy/dx->reference
        return psi,x,y
